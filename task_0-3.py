...
class TeeWriter:
    """Дублирует вывод в консоль И в файл одновременно."""
...
print(f"{'=' * 75}")
print(f"  ОТЧЁТ: Гибридная рекомендательная система (MovieLens 1M)")
...
print(f"Ratings: {ratings.shape[0]} записей")
print(f"Movies:  {movies.shape[0]} фильмов")
print(f"Users:   {users.shape[0]} пользователей")


print(f"\n{'=' * 75}")
print(f"  СТРУКТУРА И СТАТИСТИКА ДАННЫХ")
print(f"{'=' * 75}")
...
print(f"Уникальных фильмов:       {n_items_all}")
print(f"Всего оценок:              {n_ratings}")
print(f"Разреженность матрицы:     {sparsity:.2%}")
print(f"Средняя оценка:            {ratings['rating'].mean():.3f}")
print(f"Медиана оценок:            {ratings['rating'].median()}")
...
print(f"  min={user_stats['count'].min()}, "
...
print(f"  min={item_stats['count'].min()}, "
...
print(f"Жанры: {', '.join(sorted(all_genres))}")
...
for attr in ['K_NEIGHBORS_UB', 'K_NEIGHBORS_IB', 'TOP_X', 'TEST_SIZE', 'RATING_THRESHOLD',
             ...
             'DIVERSITY_RERANK']:
    print(f"  {attr:25s} = {getattr(cfg, attr)}")
...
print(f"Соотношение: {len(test_df)/len(df):.2%} в тесте")
...
print(f"Ненулевых: {train_matrix.nnz} "
...
print(f"Пользователей с релевантными тестовыми фильмами: {len(test_users_valid)}")
...
def print_metrics(m, title=""):
    ...
    for name, val in m.items():
        if name == 'Users_evaluated':
            print(f"  {'Пользователей оценено':30s}: {val}")
        else:
...
print(f"\n{'=' * 75}")
print(f"  ОЦЕНКА КАЧЕСТВА: USER-BASED vs ITEM-BASED vs HYBRID")
print(f"{'=' * 75}")
...
print(f"\n{'=' * 75}")
print(f"  ABLATION STUDY: ВКЛАД КАЖДОГО КОМПОНЕНТА ГИБРИДА")
print(f"{'=' * 75}")
...
for a in alpha_values:
    ...
    print(f"  alpha={a:.1f}: P={m['Precision@K']:.4f}, "
...
print(f"\n{'=' * 75}")
print(f"  ЭКСПЕРИМЕНТ 2: K (число соседей)")
print(f"{'=' * 75}")
...
for k in k_values:
    ...
    print(f"  k={k:3d}: P={m['Precision@K']:.4f}, "
...
for tx in topx_values:
    ...
    print(f"  top={tx:3d}: P={m['Precision@K']:.4f}, "
          f"R={m['Recall@K']:.4f}, NDCG={m['NDCG@K']:.4f}")
...
print(f"\n{'=' * 75}")
print(f"  ЭКСПЕРИМЕНТ 4: INTERSECTION BOOST")
print(f"{'=' * 75}")
...
for b in boost_values:
    ...
    print(f"  boost={b:.1f}: P={m['Precision@K']:.4f}, "
          f"NDCG={m['NDCG@K']:.4f}")
...
# --- 12.5 Test Size ---
print(f"\n{'=' * 75}")
print(f"  ЭКСПЕРИМЕНТ 5: TEST SIZE")
print(f"{'=' * 75}")
...
for ts in test_sizes:
    ...
    results_ts.append(m)
    print(f"  test_size={ts:.1f}: P={m['Precision@K']:.4f}, "
          f"NDCG={m['NDCG@K']:.4f}")
...
plt.show()
print("Графики сохранены: hybrid_analysis.png")
...
print(f"  results_ablation.csv")
print(f"  results_alpha.csv")
print(f"  results_k.csv")
print(f"  results_topx.csv")
print(f"  results_boost.csv")
print(f"  results_testsize.csv")
